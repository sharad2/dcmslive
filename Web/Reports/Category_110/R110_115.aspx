<%@ Page Title="In Process Report - Customer PO Detail Customer DC Wise" Language="C#"
 ...
 *  $Revision: 7066 $
 *  $Author: skumar $
 *  $HeadURL: http://server/svn/dcmslive/Projects/DcmsLive/trunk/Web/Reports/Category_110/R110_115.aspx $
 *  $Id: R110_115.aspx 7066 2014-07-19 11:58:58Z skumar $
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    This report displays the number of boxes in different areas for different Customer DC of passed PO, customer and virtual warehouse. 
    It shows the un-started pieces, expected pieces, packed pieces, and cancelled pieces of all customer 
    dc for passed customer, PO and virtual warehouse. It also shows progress of order 
    (% packed pieces verses ordered pieces) for passed customer, PO and virtual warehouse. 
    The user has option to filter out the data on the basis of specified Pickslip, Label and Bucket. This report only consider un-transferred and un-cancelled orders." />
    <meta name="ReportId" content="110.115" />
    <meta name="Browsable" content="false" />
    <meta name="Version" content="$Id: R110_115.aspx 7066 2014-07-19 11:58:58Z skumar $" />
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
        ...
        <jquery:JPanel ID="JPanel1" runat="server" HeaderText="Filters">
            <eclipse:TwoColumnPanel ID="tcpFilters" runat="server">
                ...
                <i:TextBoxEx ID="tbNoDays" runat="server" QueryString="no_of_days" Text="60" ToolTip="Pass the number of days. Report will show the information from the days passed till today. By default 60 no. of days.">
                    <Validators>
                        <i:Required />
                        <i:Value ValueType="Integer" Max="1800" Min="1" />
                    </Validators>
                </i:TextBoxEx>
    ...
    <oracle:OracleDataSource ID="ds" runat="server" ConnectionString="<%$ ConnectionStrings:DCMSLIVE %>"
        ...
        SELECT DISTINCT oi.customer_id AS customer_id,
                        ...
                        SUM(oi.no_of_box) over(PARTITION BY oi.customer_id, oi.vwh_id, oi.IA_ID,oi.po_id,oi.customer_dc_id) AS no_of_box,
                        SUM(oi.current_pieces) over(PARTITION BY oi.customer_id, oi.vwh_id,oi.po_id,oi.customer_dc_id) AS current_pieces,
                        SUM(oi.can_pieces) over(PARTITION BY oi.customer_id, oi.vwh_id,oi.po_id,oi.customer_dc_id) AS can_pieces,
                        SUM(OI.VAS_PIECES) OVER(PARTITION BY OI.CUSTOMER_ID, OI.VWH_ID,oi.po_id,oi.customer_dc_id) AS VAS_PIECES,
                        CASE
                          WHEN oi.ia_id IS NULL THEN
                           -100
                          ELSE
                        ...
                        SUM(oi.started_pieces) over(PARTITION BY oi.customer_id, oi.vwh_id,oi.po_id,oi.customer_dc_id) AS started_pieces,
                        SUM(oi.unstarted_pieces) over(PARTITION BY oi.customer_id, oi.vwh_id,oi.po_id,oi.customer_dc_id) AS unstarted_pieces,
    ...
    <jquery:GridViewEx ID="gv" runat="server" AllowSorting="false" DataSourceID="ds" AutoGenerateColumns="false"
        ...
        <Columns>
            ...
            <eclipse:SiteHyperLinkField DataTextField="dc_id" HeaderText="Customer DC Id" AppliedFiltersControlID="ctlButtonBar$af"
                SortExpression="dc_id" DataNavigateUrlFields="dc_id" DataNavigateUrlFormatString="R110_116.aspx?&customer_dc_id={0}" />
            <%-- <jquery:MatrixField DataHeaderFields="short_description" HeaderText="No. Of Boxes"
                DataValueFields="no_of_box, box_ia_id,VWH_ID,dc_id" DataTotalFormatString="{0:N0}"
                DisplayColumnTotals="true" DataMergeFields="customer_id,vwh_id,po_id,dc_id" DataHeaderSortFields="process_flow_sequence">
                <ItemTemplate>
                    <eclipse:SiteHyperLink ID="SiteHyperLink" runat="server" SiteMapKey="R110_114.aspx"
                        Text='<%# MatrixBinder.Eval("no_of_box", "{0:N0}")%>' NavigateUrl='<%# string.Format("customer_id={0}&po_id={1}&pickslip_id={2}&label_id={3}&bucket_id={4}&no_of_days={5}&box_ia_id={6}&vwh_id={7}&customer_dc_id={8}",MatrixBinder.Eval("customer_id"),tbPo.Text,tbPickslip.Text,lsLabel.Value,tbBucket.Text,tbNoDays.Text,MatrixBinder.Eval("box_ia_id"),MatrixBinder.Eval("VWH_ID"),MatrixBinder.Eval("dc_id"))%>'>
                    </eclipse:SiteHyperLink>
                </ItemTemplate>
            </jquery:MatrixField>--%>
            <m:MatrixField DataHeaderFields="short_description" DataCellFields="no_of_box, box_ia_id,VWH_ID,dc_id"
                HeaderText="No. Of Boxes" DataMergeFields="customer_id,vwh_id,po_id,dc_id">
                <MatrixColumns>
                    <m:MatrixColumn ColumnType="CellValue" ColumnTotalFormatString="{0:N0}" DisplayColumnTotal="true">
                        <ItemTemplate>
                            <eclipse:SiteHyperLink ID="SiteHyperLink" runat="server" SiteMapKey="R110_114.aspx"
                                Text='<%# Eval("no_of_box", "{0:N0}")%>' NavigateUrl='<%# string.Format("customer_id={0}&po_id={1}&pickslip_id={2}&label_id={3}&bucket_id={4}&no_of_days={5}&box_ia_id={6}&vwh_id={7}&customer_dc_id={8}&vas_id={9}",Eval("customer_id"),tbPo.Text,tbPickslip.Text,lsLabel.Value,tbBucket.Text,tbNoDays.Text,Eval("box_ia_id"),Eval("VWH_ID"),Eval("dc_id"),cbVAS.Value)%>'>
                            </eclipse:SiteHyperLink>
                        </ItemTemplate>
                    </m:MatrixColumn>
